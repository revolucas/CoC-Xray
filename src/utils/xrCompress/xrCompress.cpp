...
xrCompressor::xrCompressor()
	:fs_pack_writer(NULL), bnoCompress(true), files_list(NULL), folders_list(NULL), bStoreFiles(false), pPackHeader(NULL), config_ltx(NULL)
{
	bytesSRC		= 0;
	bytesDST		= 0;
	filesTOTAL		= 0;
	...
	XRP_MAX_SIZE	= 1024*1024*1024; // bytes (640Mb)
...
bool xrCompressor::testVFS(LPCSTR path)
{
	...
	string256			p_ext;
	string256			p_fname;
	_splitpath(path, 0, 0, p_fname, p_ext);
...
#else
	strconcat		(sizeof(fname),fname,tgt_folder,".db",itoa(num,s_num,10));
#endif
	unlink			(fname);
...
void xrCompressor::PerformWork()
{
	if (!files_list->empty() && target_name.size())
	{
		...
		{
			xr_sprintf				(caption,"Compress files: %d/%d - %d%%",it+1,files_list->size(),(it*100)/(files_list->size()-1));
