...
CHelicopter::CHelicopter()
{
	...
	active_camera = 0;
	camera[eacFirstEye] = xr_new<CCameraFirstEye>(this, CCameraBase::flRelativeLink | CCameraBase::flPositionRigid);
	camera[eacFirstEye]->tag = eacFirstEye;
	camera[eacFirstEye]->Load("heli_firsteye_cam");

	camera[eacLookAt] = xr_new<CCameraLook>(this, CCameraBase::flRelativeLink);
	camera[eacLookAt]->tag = eacLookAt;
	camera[eacLookAt]->Load("heli_look_cam");

	camera[eacFreeLook] = xr_new<CCameraLook>(this);
